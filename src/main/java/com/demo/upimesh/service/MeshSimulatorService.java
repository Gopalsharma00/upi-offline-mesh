...
public class MeshSimulatorService {
    ...
    public boolean addDevice(String id, boolean hasInternet) {
        if (devices.containsKey(id)) return false;
        devices.put(id, new VirtualDevice(id, hasInternet));
        log.info("Added device {} (internet={})", id, hasInternet);
        return true;
    }
    ...
    public void toggleDeviceInternet(String deviceId) {
        VirtualDevice device = devices.get(deviceId);
        if (device != null) {
            device.setHasInternet(!device.hasInternet());
            log.info("Toggled internet for device {}: {}", deviceId, device.hasInternet());
        }
    }
     ...
     */
    public GossipResult gossipOnce() { return gossipOnce(null); }

    public GossipResult gossipOnce(Map<String, List<String>> neighbors) {
        int transfers = 0;
        ...
        for (VirtualDevice src : deviceList) {
            for (MeshPacket pkt : snapshot.get(src.getDeviceId())) {
                ...
                List<VirtualDevice> targets;
                if (neighbors != null && neighbors.containsKey(src.getDeviceId())) {
                    List<String> nbrs = neighbors.get(src.getDeviceId());
                    targets = deviceList.stream().filter(d -> nbrs.contains(d.getDeviceId())).toList();
                } else if (neighbors != null) {
                    targets = List.of();
                } else {
                    targets = deviceList;
                }
                for (VirtualDevice dst : targets) {
                    if (dst == src) continue;
        ...
        log.info("Gossip round complete: {} transfers", transfers);
