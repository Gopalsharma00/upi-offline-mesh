...
public class ApiController {
    ...
    public Map<String, Object> meshGossip(@RequestBody(required = false) Map<String, List<String>> neighbors) {
        MeshSimulatorService.GossipResult r = mesh.gossipOnce(neighbors);
        return Map.of(
    ...
    @PostMapping("/mesh/device/{deviceId}/toggle-internet")
    public Map<String, Object> toggleInternet(@PathVariable String deviceId) {
        mesh.toggleDeviceInternet(deviceId);
        VirtualDevice d = mesh.getDevice(deviceId);
        return Map.of("deviceId", deviceId, "hasInternet", d != null && d.hasInternet());
    }

    @PostMapping("/mesh/device/add")
    public Map<String, Object> addDevice(@RequestBody Map<String, Object> body) {
        String id = (String) body.get("deviceId");
        boolean hasInternet = Boolean.TRUE.equals(body.get("hasInternet"));
        boolean added = mesh.addDevice(id, hasInternet);
        return Map.of("deviceId", id, "added", added);
    }

    @PostMapping("/mesh/device/{deviceId}/remove")
    public Map<String, Object> removeDevice(@PathVariable String deviceId) {
        boolean removed = mesh.removeDevice(deviceId);
        return Map.of("deviceId", deviceId, "removed", removed);
    }
    ...
    public Map<String, Object> meshFlush() {
        ...
        uploads.parallelStream().forEach(up -> {
            try {
                ...
                resMap.put("bridgeNode", up.bridgeNodeId());
                
                String pId = up.packet().getPacketId();
                resMap.put("packetId", (pId != null && pId.length() >= 8) ? pId.substring(0, 8) : "unknown");
                
                resMap.put("outcome", r.outcome());
                resMap.put("hopCount", 5 - up.packet().getTtl());
                resMap.put("reason", r.reason() == null ? "" : r.reason());
                resMap.put("transactionId", r.transactionId() == null ? -1 : r.transactionId());

                synchronized (results) {
                    results.add(resMap);
                }
            } catch (Exception e) {
                log.error("Error processing bridge upload: {}", e.getMessage());
            }
        });
